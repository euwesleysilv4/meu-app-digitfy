...
CREATE OR REPLACE FUNCTION public.update_plan_direct_v2(user_id UUID, new_plan TEXT)
RETURNS VOID AS $$
BEGIN
    -- Atualização ultra direta que evita qualquer gatilho ou validação que cause problemas
    EXECUTE 'UPDATE public.profiles SET plano = $1::user_plan, data_modificacao = NOW() WHERE id = $2'
...
COMMENT ON FUNCTION public.update_plan_direct_v2 IS 'Atualiza diretamente o plano de um usuário contornando gatilhos ou validações problemáticas (v2)';
...
-- Comentário para documentação
COMMENT ON FUNCTION public.execute_sql_safe IS 'EMERGÊNCIA APENAS: Executa SQL nativo, use com extrema cautela apenas em situações urgentes';
...
CREATE OR REPLACE FUNCTION public.reset_and_update_plan_v2(user_id UUID, new_plan TEXT)
RETURNS BOOLEAN AS $$
DECLARE
    user_data RECORD;
    result BOOLEAN;
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        ...
        BEGIN
            ...
            VALUES (
                user_data.id, user_data.email, user_data.nome, new_plan::user_plan, user_data.role,
                NOW(), NOW(), 'online', TRUE
            );
...
-- Comentário para documentação
COMMENT ON FUNCTION public.reset_and_update_plan_v2 IS 'Última medida de emergência: pode redefinir o perfil para atualizar o plano (v2)';
