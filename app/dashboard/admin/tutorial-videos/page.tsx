...
export default function TutorialVideosPage() {
  ...
  const fetchVideos = async () => {
    setLoading(true);
    try {
      let query = supabase.from('tutorial_videos').select('*');
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.rpc('delete_tutorial_video', {
        video_id: id
      });
  ...
  const toggleStatus = async (video: TutorialVideo) => {
    try {
      const { error } = await supabase.rpc('update_tutorial_video', {
        video_id: video.id,
  ...
  const handleSaveVideo = async (video: TutorialVideo) => {
    try {
      if (currentVideo) {
        ...
        // Adicionar novo vídeo
        const { error } = await supabase.rpc('add_tutorial_video', {
          titulo_param: video.titulo,
          descricao_param: video.descricao,
          youtube_id_param: video.youtube_id,
          categoria_param: video.categoria,
